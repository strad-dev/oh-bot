...
import static events.CommandListener.finishAnalysis;
import static events.CommandListener.isPeakHours;
...
public class CommandListener extends ListenerAdapter {
		...
		if(!e.getAuthor().isBot() && e.getMessage().getContentRaw().startsWith("!")) {
			...
			if(message[0].equals("analyze") && e.getMember().hasPermission(Permission.ADMINISTRATOR)) {
				if(!isAnalysisRunning) {
					...
					CreateAnalysisThread.setEvent(e);
					Thread thread = new Thread(new CreateAnalysisThread());
					thread.start();
				} else {
	...
	static void finishAnalysis() {
		isAnalysisRunning = false;
	}

	/**
	 * Determines if the given hour is a "peak hour" (i.e. there are TAs available on demand)
	 *
